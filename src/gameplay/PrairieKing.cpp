...
void PrairieKing::Dracula::Draw(const Texture2D &texture, Vector2 topLeftScreenCoordinate)
{
        ...
        phase == SUMMON_DEMON_PHASE || phase == SUMMON_MUMMY_PHASE)
    {
        ...
        if (phase == GLOATING_PHASE)
        {
            ...
            DrawTexturePro(
                texture,
                Rectangle{480.0f, 80.0f, 32.0f, 32.0f},
                Rectangle{
    ...
    else
    {
        DrawTexturePro(
            texture,
            Rectangle{464.0f + (phaseInternalTimer / 100 % 2) * 16.0f, 64.0f, 16.0f, 16.0f},
            Rectangle{topLeftScreenCoordinate.x + position.x, topLeftScreenCoordinate.y + position.y, position.width, position.height},
            Vector2{0, 0},
            0.0f,
            WHITE);
...
void PrairieKing::Outlaw::Draw(const Texture2D &texture, Vector2 topLeftScreenCoordinate)
{
    ...
    if (flashColorTimer > 0.0f)
    {
        DrawTexturePro(
            texture,
            Rectangle{368.0f, 48.0f, 16.0f, 16.0f},
            Rectangle{topLeftScreenCoordinate.x + position.x, topLeftScreenCoordinate.y + position.y, position.width, position.height},
            Vector2{0, 0},
            0.0f,
            WHITE);
    ...
    if (phase == TALKING_PHASE || phase == HIDING_PHASE)
    {
        DrawTexturePro(
            texture,
            Rectangle{432.0f + ((phaseCountdown / 250 % 2 == 0) ? 16.0f : 0.0f), 128.0f, 16.0f, 16.0f},
            Rectangle{topLeftScreenCoordinate.x + position.x, topLeftScreenCoordinate.y + position.y, position.width, position.height},
            Vector2{0, 0},
            0.0f,
            WHITE);
        ...
        if (phase == TALKING_PHASE && phaseCountdown > 1000)
        {
            ...
            DrawTexturePro(
                texture,
                Rectangle{448.0f + ((whichWave > 5) ? 32.0f : 0.0f), 144.0f, 32.0f, 32.0f},
                Rectangle{
    ...
    else
    {
        DrawTexturePro(
            texture,
            Rectangle{464.0f + ((phaseCountdown / 80 % 2 == 0) ? 16.0f : 0.0f), 128.0f, 16.0f, 16.0f},
            Rectangle{topLeftScreenCoordinate.x + position.x, topLeftScreenCoordinate.y + position.y, position.width, position.height},
            Vector2{0, 0},
            0.0f,
            WHITE);
...
void PrairieKing::Update(float deltaTime)
{
    ...
    // Actualizar duración de powerups activos
    ...
    // Actualizar powerups en el suelo
    ...
    if (!m_gameOver && !m_onStartMenu)
    {
        ...
        else
        {
            ...
            // Manejar la música del overworld
